...
class WorkflowBuilder:
    def __init__(self, workflow_definition: WorkflowDefinition, llm_config: Dict[str, str]):
    ...
    def build_and_validate(self) -> Supervisor:
        ...
        if main_sup_builder.validate():
            main_supervisor = main_sup_builder.build()
        ...
        for asst_sup_def in self.definition.assistant_supervisors:
            ...
            if asst_sup_builder.validate():
                asst_supervisor = asst_sup_builder.build()
...
{tool_name}_tool = {{"tool": {tool_name}, "metadata": {metadata_var}}}
    ...
    def _generate_agent_definitions(self) -> str:
        ...
        for agent_def in self.definition.agents:
            tools_list = []
            if agent_def.tools:
                tools_list = [f"{tool.metadata.function.name}_tool" 
                            for tool in agent_def.tools]
            
            # Format output schema if provided
            output_schema_str = (
                f"json.loads('''{agent_def.output_schema}''')"
                if agent_def.output_schema else "None"
            )
